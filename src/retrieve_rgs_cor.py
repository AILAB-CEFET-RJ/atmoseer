...
def corrige_txt(station_name, years, months):
    for year in years:
        for month in months:
            file = "./data/RAW_data/COR/pluviometrica/" + station_name + "_" + year + month + "_Plv.txt"
            if os.path.exists(file):
                fin = open(file, "rt")
                if not os.path.exists("./data/RAW_data/COR/pluviometrica/aux_"+ station_name):
                    os.mkdir("./data/RAW_data/COR/pluviometrica/aux_"+ station_name)
                fout = open("./data/RAW_data/COR/pluviometrica/aux_" + station_name + "/" + station_name + "_" + year + month + "_Plv2.txt", "wt")
                count = 0
                for line in fin:
                    count += 1
                    #fout.write(re.sub('\s+', ' ', line.replace(':00      ', ':00   HBV')))
                    text_to_write = re.sub('\s+', ' ', line.replace('HBV', '').replace('05 min', '05_min').replace('10 min', '10_min').replace('15 min', '15_min').replace('01 h', '01_h').replace('04 h', '04_h').replace('24 h', '24_h').replace('96 h', '96_h') )
...
def import_data(station_name, years, months, arg_end):
    ...
    for year in years:
        check = 0
        for month in months:
            texto = './data/RAW_data/COR/pluviometrica/aux_' + station_name + '/' + station_name + '_' + year + month + '_Plv2.txt'
            if os.path.exists(texto):
                ...
                if len(df.columns) == 9 or len(df.columns) == 10:
                    if len(df.columns) == 10:
                        df.columns = ['Dia', 'Hora', '05 min', '10 min', '15 min', '01 h', '04 h', '24 h', '96 h','teste']
                        del df['05 min']
                        del df['10 min']
                        del df["teste"]
                    else:
                        df.columns = ['Dia', 'Hora', '05 min', '10 min', '15 min', '01 h', '04 h', '24 h', '96 h']
                        del df['05 min']
                        del df['10 min']
                else:
                    if len(df.columns) == 8:
                        df.columns = ['Dia', 'Hora', '15 min', '01 h', '04 h', '24 h', '96 h','teste']
                        del df["teste"]
                    else:
                        df.columns =  ['Dia', 'Hora', '15 min', '01 h', '04 h', '24 h', '96 h']
                df['15 min'] = df['15 min'][~df['15 min'].isin(['-', 'ND'])].astype(float)
            ...
            if os.path.exists(texto):
                ...
                if len(data2.columns) == 9 or len(data2.columns) == 10:
                    if len(data2.columns) == 10:
                        data2.columns = ['Dia', 'Hora', '05 min', '10 min', '15 min', '01 h', '04 h', '24 h', '96 h','teste']
                        del data2['05 min']
                        del data2['10 min']
                        del data2["teste"]
                    else:
                        data2.columns = ['Dia', 'Hora', '05 min', '10 min', '15 min', '01 h', '04 h', '24 h', '96 h']
                        del data2['05 min']
                        del data2['10 min']
                else:
                    if len(data2.columns) == 8:
                        data2.columns = ['Dia', 'Hora', '15 min', '01 h', '04 h', '24 h', '96 h','teste']
                        del data2["teste"]
                    else:
                        data2.columns =  ['Dia', 'Hora', '15 min', '01 h', '04 h', '24 h', '96 h']
